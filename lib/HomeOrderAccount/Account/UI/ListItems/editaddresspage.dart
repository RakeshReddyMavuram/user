...
class EditAddresspage extends StatefulWidget {
  ...
  EditAddresspage(this.pincode, this.houseno, this.address, this.state, this.address_id, this.vendorid,this.city_id,this.area_id,this.type);
...
class EditAddresspageState extends State<EditAddresspage> {
  ...
  void getCityList() async {
    ...
    // dynamic vId = prefs.getString('vendor_id');
    // print('vid $vId');
    var url = cityList;
    http.post(url, body: {
      ...
      if (value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          if(tagObjs!=null && tagObjs.length>0){
            ...
            List<CityList> tagObjs1 =  tagObjs.where((element) => element.city_id.toString() == '${widget.city_id}').toList();
            if(tagObjs1!=null && tagObjs1.length>0){
              setState(() {
                selectCity = tagObjs1[0].city_name;
                selectCityId = tagObjs1[0].city_id;
                streetController.text = streetController.text.replaceAll('${selectCity},', '');
                streetController.text = streetController.text.replaceAll('${selectCity}', '');
              });
              if(selectCityId!=null && selectCityId!='' && selectCityId!=null){
      ...
      if (value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          if(tagObjs!=null && tagObjs.length>0){
            ...
            List<AreaList> tagObjs1 = tagObjs.where((element) => element.area_id.toString()=='${widget.area_id}').toList();
            if(tagObjs1!=null && tagObjs1.length>0){
              setState(() {
                selectAreaId = tagObjs1[0].area_id;
                selectArea = tagObjs1[0].area_name;
                streetController.text = streetController.text.replaceAll(',${selectArea},', '');
                streetController.text = streetController.text.replaceAll('${selectArea},', '');
                streetController.text = streetController.text.replaceAll('${selectArea}', '');
              });
            }
