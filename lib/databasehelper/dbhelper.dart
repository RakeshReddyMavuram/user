...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> getPharmaProduct() async {
    ...
    return await db.rawQuery('SELECT $pharmatable.$varientId, $pharmatable.$addQnty, $pharmatable.$quantitiy, $pharmatable.$unit, $pharmatable.$price, $pharmatable.$productName FROM $pharmatable');
  }
  ...
  Future<int> deleteAddOnIdPharma(dynamic id) async {
    Database db = await instance.database;
    return await db
        .delete(pharmaadon, where: '$addonid = ?', whereArgs: [id]);
  }

  Future<int> deleteAddOnIdPharmaWithVid(dynamic id, dynamic vid) async {
    Database db = await instance.database;
    // return await Sqflite.firstIntValue(await db.rawQuery("DELETE FROM $pharmaadon WHERE $varientId=$vid AND $addonid=$id"));
    return await db.delete(pharmaadon, where: '$addonid = ? AND $varientId = ?', whereArgs: [id,vid]);
  }

  Future<int> getPharmaCount(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(
        await db.rawQuery("SELECT COUNT(*) FROM $pharmatable WHERE $varientId=$id"));
    return count;
  }
  ...
  Future<int> deletePharmaProduct(dynamic id) async {
    Database db = await instance.database;
    return await db.delete(pharmatable, where: '$varientId = ?', whereArgs: [id]);
  }
  ...
  Future<int> updatePharmaProductData(Map<String, dynamic> row, dynamic vat) async {
    Database db = await instance.database;
//    int id = row[vat];
    return await db
  ...
  Future<int> queryRowPharmaCount() async {
    Database db = await instance.database;
    return Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM $pharmatable'));
  }

  Future<int> getVarientPharmaCount(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(
        await db.rawQuery("SELECT $addQnty FROM $pharmatable WHERE $varientId=$id"));
    return count;
  }
  ...
  Future<List<Map<String, dynamic>>> getFaviourteProduct(dynamic id) async {
    Database db = await instance.database;
    return await db.rawQuery('SELECT $varientId FROM $faviourteProdcutRest WHERE $productId=$id');
  }
  ...
  Future<List<Map<String, dynamic>>> getAddOnList(dynamic id) async {
    Database db = await instance.database;
    return await db.rawQuery('SELECT $addonid FROM $addontable WHERE $varientId=$id');
  }
  ...
  Future<List<Map<String, dynamic>>> getResturantOrderList() async {
    Database db = await instance.database;
    return await db.rawQuery('SELECT $resturantOrder.$varientId, $resturantOrder.$addQnty, $resturantOrder.$quantitiy, $resturantOrder.$unit, $resturantOrder.$price, $resturantOrder.$productName FROM $resturantOrder');
  }
  ...
  Future<int> getCountAddon(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(await db.rawQuery(
        "SELECT COUNT(*) FROM $addontable WHERE $addonid=$id"));
    return count;
  }
  ...
  Future<int> queryRowCountRest() async {
    Database db = await instance.database;
    return Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM $resturantOrder'));
  }

  Future<int> queryRowBothCount() async {
    Database db = await instance.database;
    int counttable1 = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM $table'));
    int counttable2 = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM $resturantOrder'));
...
//    int id = row[vat];
  ...
  Future<int> updateRestProductData(Map<String, dynamic> row, dynamic vat) async {
    Database db = await instance.database;
//    int id = row[vat];
    return await db
  ...
  Future<int> deleteResProduct(dynamic id) async {
    Database db = await instance.database;
    return await db.delete(resturantOrder, where: '$varientId = ?', whereArgs: [id]);
  }

  Future<int> deleteResturant(dynamic id) async {
    Database db = await instance.database;
    return await db.delete(faviourteResturant, where: '$vendor_id = ?', whereArgs: [id]);
  }
  ...
  Future<int> deleteAddOnId(dynamic id) async {
    Database db = await instance.database;
    return await db
        .delete(addontable, where: '$addonid = ?', whereArgs: [id]);
  }
  ...
  Future<int> getRestProductcount(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(
        await db.rawQuery("SELECT COUNT(*) FROM $resturantOrder WHERE $varientId=$id"));
    return count;
  }
  ...
  Future<int> getRestProdByProId(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(
        await db.rawQuery("SELECT COUNT(*) FROM $resturantOrder WHERE $productId=$id"));
    return count;
  }
  ...
  Future<int> getRestProdQty(dynamic id) async {
    Database db = await instance.database;
    int count = Sqflite.firstIntValue(
        await db.rawQuery("SELECT $addQnty FROM $resturantOrder WHERE $varientId=$id"));
    return count;
  }
  ...
  Future<dynamic> calculateTotalRest() async {
    Database db = await instance.database;
    var result = db.rawQuery("SELECT SUM($price) as Total FROM $resturantOrder");
    return result;
  }
  ...
  Future<dynamic> calculateTotalRestAdonA(dynamic id) async {
    Database db = await instance.database;
    var result2 = db.rawQuery("SELECT SUM($price) as Total FROM $addontable WHERE $varientId=$id");
    // print('T--${result.toString()}');
    // print('T--${result2.toString()}');
    // dynamic totalAmount_1 = 0.0;
    // dynamic totalAmount_12 = 0.0;
    // if (result != null) {
    //   var tagObjsJson = result as List;
    //   print('T--${tagObjsJson.toString()}');
    //   totalAmount_1 = tagObjsJson[0]['Total'];
    // }
    // if (result2 != null) {
    //   var tagObjsJson = result2 as List;
    //   print('T--${tagObjsJson.toString()}');
    //   totalAmount_12 = tagObjsJson[0]['Total'];
    // }
