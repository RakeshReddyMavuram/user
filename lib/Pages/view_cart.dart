...
class _ViewCartState extends State<ViewCart> {
  ...
  List<String> order = [labelDate, labelMonth, labelWeekDay];
  ...
  Color selectedDecorationColor = kHintColor;
...
//     });
    ...
    initialSelectedDates = feedInitialSelectedDates(minSelectedDateCount, 9);
    getStoreName();
    getCartItem();
    getCatC();
    dynamic date =
                        ...
                        itemBuilder: (context, index) {
                          return cartOrderItemListTile(
                            ...
                            cartListI[index].unit,
                              ...
                              cartListI[index].add_qnty--;
                              cartListI[index].price =
                              (price_d * cartListI[index].add_qnty);
                              addOrMinusProduct(
                                  cartListI[index].product_name,
                                  cartListI[index].unit,
                                  cartListI[index].price,
...
//                  dateTimeSt = '${dateTime.year}-${dateTime.month}-${dateTime.day}';
                          dateTimeSt =
                          '${dateTime.year}-${(dateTime.month.toString().length == 1) ? '0' + dateTime.month.toString() : dateTime.month}-${dateTime.day}';
                          dynamic date =
                              '${dateTime.day}-${(dateTime.month.toString().length == 1) ? '0' + dateTime.month.toString() : dateTime.month}-${dateTime.year}';
                          hitDateCounter(date);
                          print(' date time - $dateTimeSt');
                        },
                        ...
                        dateTextStyle: TextStyle(color: kMainTextColor, fontSize: 18),
                        monthTextStyle:
                        TextStyle(color: kMainTextColor, fontSize: 18),
                        weekDayTextStyle:
                        TextStyle(color: kMainTextColor, fontSize: 18),
                    ...
                    (!isFetchingTime&&radioList.length>0)
                      ...
                      child: Row(
                        ...
                        children: [
                          isFetchingTime?CircularProgressIndicator():Container(width: 0.5,),
                          isFetchingTime?SizedBox(
                            width: 10,
                          ):Container(width: 0.5,),
                          Text(
                            (isFetchingTime)?'Fetching time slot':'No time slot present now check other date..',
                            style: TextStyle(
                                fontSize: 18,
      ...
      if(radioList!=null &&radioList.length>0){
        if (totalAmount != null && totalAmount > 0.0 && addressDelivery != null) {
          ...
          for (CartItem item in cartListI) {
            orderArray.add(OrderArrayGrocery(int.parse('${item.add_qnty}'), int.parse('${item.varient_id}')));
          }
          print(
              '$userId $vendorId ${orderArray.toString()} ${dateTimeSt} ${radioList[radioId]}');
          http.post(url, body: {
            'user_id': userId.toString(),
            'vendor_id': vendorId,
            'order_array': orderArray.toString(),
            'delivery_date': dateTimeSt,
            'time_slot': '${radioList[radioId]}',
            'ui_type': ui_type
          }).then((value) {
            print('${value.statusCode} ${value.body}');
...
//        print('resp value - ${value.body}');
          ...
          if (addressDelivery != null) {
            ...
            Toast.show(
                'Please add your delivery address to continue shopping..', context,
                duration: Toast.LENGTH_SHORT);
          }
        }
      }
        ...
        Toast.show('Please select a delivery time to continue!', context, duration: Toast.LENGTH_SHORT);
      }
    }else{
      setState(() {
        showDialogBox = false;
      });
      Toast.show('Please add some items into cart!', context, duration: Toast.LENGTH_SHORT);
    }
      ...
      if (value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          Navigator.of(context).push(MaterialPageRoute(builder: (context) {
            return PaymentPage(vendorId, details.order_id, details.cart_id,
                double.parse(details.total_price.toString()),tagObjs,orderArray);
          }));
        }
      }
    }).catchError((e) {
...
//        else{
//          cartListI[index].add_qnty = itemCount;
//        }
      });
    });
  }
                ...
                child: Row(
                    ...
                    children: <Widget>[
                      Container(
                        ...
                        child: Text('${qnty} ${unit}',style: Theme.of(context).textTheme.caption,),
                      ),
                      Spacer(),
                      Container(
                        height: 30.0,
