...
class _RestuarantViewCartState extends State<RestuarantViewCart> {
  ...
  String storeName = '';
  String storeTimeing = '',storopen,storeclose;
  String vendorCatId = '';
  String uiType = '';
  dynamic vendorId = '';
  ...
  void getStoreName() async {
    ...
    String storename = prefs.getString('store_resturant_name');
     storopen = prefs.getString('store_resturant_open');
     storeclose = prefs.getString('store_resturant_close');
    ...
    setState(() {
      ...
      if (storename != null && storename.length > 0) {
        storeName = storename;
        storeTimeing='we accept orders from '+storopen+' to '+storeclose;
      }
        ...
        if (jsonData['status'] == "1" && jsonData['data']!=null && jsonData['data']!='null') {
          ...
          if(addressWelcome.data!=null){
            setState(() {
              ...
              deliveryCharge = deliveryCharge = double.parse('${addressDelivery.delivery_charge}');
              getCatC();
            });
          }else{
            setState(() {
                      ...
                      child: ListView(
                        ...
                        children: <Widget>[
                          ...
                          Container(
                            ...
                            color: kCardBackgroundColor,
                            child: Text(storeTimeing,
                                style: Theme.of(context)
                                    .textTheme
                                    .headline6
                                    .copyWith(
                                    color: Color(0xff13800b),
                                    letterSpacing: 0.4)),
                          ),
                                      ...
                                      GestureDetector(
                                        onTap: () async{
                                           await SharedPreferences.getInstance().then((value){
                                            String vendorId = value.getString('res_vendor_id');
    ...
    client.post(url, body: {'vendor_id': '${vendorId}'}).then((value) {
      ...
      if (value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          var myhour = new DateTime(now.hour).toString();
          var mymin = new DateTime( now.minute).toString();


          var parts = storeclose.split(':');
          int storeclosehour = int.parse(parts[0].trim());
          assert(storeclosehour is int);
          int storeclosemin = int.parse(parts[1].trim());
          assert(storeclosemin is int);

          var parts_open = storopen.split(':');
          int storeopenhour = int.parse(parts_open[0].trim());
          assert(storeopenhour is int);
          var storeopenmin = int.parse(parts_open[1].trim());
          assert(storeopenmin is int);


          int myhourint = int.parse(myhour);
          assert(myhourint is int);

          int myminint = int.parse(mymin);
          assert(myminint  is int);

          if(myhourint>storeopenhour && myhourint<storeclosehour){
            ...
            Toast.show("Not accepting orders now", context,
                duration: Toast.LENGTH_SHORT);
          }

          if(myhourint==storeopenhour){
            if(myminint>=storeopenmin){
              Navigator.of(context).push(MaterialPageRoute(builder: (context) {
                return PaymentRestPage(vendorId, details.order_id, details.cart_id,
                    double.parse(details.total_price.toString()), tagObjs);
              }));
