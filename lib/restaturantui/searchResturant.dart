...
//                   nearStores = nearStoresSearch.where((element) => element.vendor_name.toString().toLowerCase().contains(value.toLowerCase())).toList();
                              ...
                              return GestureDetector(
                                ...
                                child: Material(
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      children: [
                                        (categoryList[index].cat_image!=null)?Image.network(
                                          imageBaseUrl+categoryList[index].cat_image,
                                          height: 80,
                                          width: 90,
                                        ):Image.asset(
                            ...
                            return Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Container(
                                    color: kWhiteColor,
                                    child: JuiceListSearch(item,widget.currencySymbol,(){
                                      getCartCount();
                                    }),
                                  ),
                                  Container(
  ...
  hitNavigator(BuildContext context, vendor_name, vendor_id, distance, StoresRestaurant nearStor) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("res_vendor_id", '${vendor_id}');
    prefs.setString("store_resturant_name", '${vendor_name}');
    NearStores st = NearStores(nearStor.vendor_name, nearStor.vendor_phone, nearStor.vendor_id, nearStor.vendor_logo, nearStor.vendor_category_id, nearStor.distance, nearStor.lat, nearStor.lng, nearStor.delivery_range,nearStor.online_status,nearStor.vendor_loc,nearStor.about,nearStor.opening_time,nearStor.closing_time);
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) =>
                                      ...
                                      db.getRestProdQty('${item.variant_id}')
                                        ...
                                        db.getAddOnList('${item.variant_id}')
                                          ...
                                          if (valued != null &&
                                            ...
                                            i++) {
                                              int ind = addOnlist.indexOf(AddonList('${widget.item.addons[i].addon_id}'));
                                              if (ind != null && ind >= 0) {
                                                setState(() {
                                          ...
                                          db.calculateTotalRestAdonA('${item.variant_id}').then((value1){
                                            ...
                                            productDescriptionModalBottomSheet(
                                                ...
                                                index,
                                                addOnlist,
                                                widget.currencySymbol,priced).then((value) {
                                              // print('${value}');
...
Future productDescriptionModalBottomSheet(context, height,
    SearchProuduct item, getIn, List<AddonList> addOnlist, currencySymbol, double priced) async {
  ...
  prefs.setString("store_resturant_name", '${(item.vendor_name!=null)?item.vendor_name:''}');
  ...
  if (item.varients[getIn].addOnQty > 0) {
    price = (double.parse('${item.varients[getIn].addOnQty}') *
        double.parse('${item.varients[getIn].price}'))+priced;
  }
  double width = MediaQuery.of(context).size.width;
  DatabaseHelper db = DatabaseHelper.instance;
  db.getAddOnList('${item.varients[getIn].variant_id}')
      .then((valued) {
    List<AddonList> addOnlist = [];
    if (valued != null &&
        valued.length > 0) {
      ...
      builder: (BuildContext bc) {
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            setAddOrMinusProdcutQty(SearchVaritant items, BuildContext context, index, produtId, productName, qty) async {
              print('tb - ${qty}');
              DatabaseHelper db = DatabaseHelper.instance;
              db.getRestProductcount('${items.variant_id}').then((value) {
                ...
                if (value == 0) {
                  db.insertRaturantOrder(vae).then((valueaa) {
                    db.calculateTotalRestAdonA('${item.varients[getIn].variant_id}').then((value1){
                      double pricedd = 0.0;
                      print('${value1}');
                      if(value!=null){
                        var tagObjsJson = value1 as List;
                        ...
                        if(totalAmount_1!=null){
                          setState((){
                            ...
                            price = (double.parse('${item.varients[getIn].price}') * qty)+pricedd;
                          });
                        }else{
                          setState((){
                            item.varients[getIn].addOnQty = qty;
                  ...
                  if (qty == 0) {
                    db.deleteResProduct('${items.variant_id}').then((value2) {
                      setState(() {
                        item.varients[getIn].addOnQty = qty;
                        price = double.parse('${item.varients[getIn].price}') * qty;
                      });
            ...
            Future<dynamic> setAddOnToDatabase(isSelected, DatabaseHelper db, AddOnsL addon, variant_id, int indexaa) async {
              var vae = {
                    ...
                    if(value!=null){
                      ...
                      if(totalAmount_1!=null){
                        setState((){
                          ...
                          pricedd = double.parse('${totalAmount_1}');
                          // item.varients[getIn].addOnQty = qty;
                          price = (double.parse('${item.varients[getIn].price}') * item.varients[getIn].addOnQty)+pricedd;
                        });
                      }else{
                        setState((){
                          item.addons[indexaa].isAdd = true;
            ...
            Future<dynamic> deleteAddOn(
                isSelected, DatabaseHelper db, AddOnsL addon, variant_id, int indexaa) async {
                                          ...
                                          onTap: () async {
                                            if (item.varients[getIn].addOnQty > 0) {
                                              ...
                                              db.getCountAddon('${item.addons[indexw].addon_id}').then((value) {
                                                print('addon count $value');
                                                if (value != null && value > 0) {
                                                  deleteAddOn((item.addons[indexw].isAdd != null && item.addons[indexw].isAdd) ? true : false, db, item.addons[indexw], item.varients[getIn].variant_id,indexw)
                                                      .then((value) {
                                                    print('addon deleted $value');
                                                  }).catchError((e) {
                                                    print(e);
                                                  ...
                                                  // });
                                                  var vae = {
                                                    DatabaseHelper.varientId: '${item.varients[getIn].variant_id}',
