...
Future productDescriptionModalBottomSheet(context, height, PopularItem item, currencySymbol,priced) {
  ...
  if (item.variant[index].addOnQty > 0) {
    price = (double.parse('${item.variant[index].addOnQty}') *
        double.parse('${item.deal_price}'))+priced;
  }
  double width = MediaQuery.of(context).size.width;
  bool s = false, m = false, l = false, option1 = false, option2 = false;
DatabaseHelper db = DatabaseHelper.instance;
  db.getAddOnList('${item.variant[index].variant_id}')
      .then((valued) {
      ...
      builder: (BuildContext bc) {
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            setAddOrMinusProdcutQty(PopularItem items, BuildContext context, produtId, productName, qty) async {
              print('tb - ${qty}');
              DatabaseHelper db = DatabaseHelper.instance;
              db.getRestProductcount('${items.variant_id}').then((value) {
                ...
                var vae = {
                  ...
                  DatabaseHelper.price: ((double.parse('${items.deal_price}')* qty)),
                ...
                if (value == 0) {
                  db.insertRaturantOrder(vae).then((valueaa) {
                    setState(() {
                      ...
                      price =
                          double.parse('${item.deal_price}') * qty;
                    });
                  });
                } else {
                  if (qty == 0) {
            ...
            Future<dynamic> setAddOnToDatabase(
                isSelected, DatabaseHelper db, AddOnList addon, variant_id) async {
              var vae = {
                      ...
                      if(value!=null){
                        ...
                        if(totalAmount_1!=null){
                          setState((){
                            ...
                            if (item.variant[index].addOnQty > 0) {
                              price = (double.parse('${item.variant[index].addOnQty}') * double.parse('${item.deal_price}'))+pricedd;
                            }
                          });
                        }else{
                          setState((){
                      ...
                      children: <Widget>[
                        InkWell(
                          onTap: () async {
                            if (item.variant[index].addOnQty > 0) {
                              DatabaseHelper db = DatabaseHelper.instance;
                              db.getCountAddon('${addon.addon_id}')
                                  .then((value) {
                                ...
                                if (value != null && value > 0) {
                                  ...
                                  setAddOnToDatabase(isSelected, db, addon, variant_id).then((value) {
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            Container(
                              ...
                              child: Image.network(imageBaseUrl+item.product_image,fit: BoxFit.fill,),
                            ),
                            Container(
                      ...
                      //   color: kWhiteColor,
                      //   child:
                      //   (item.variant!=null && item.variant.length>0)?ListView.separated(
                      //     shrinkWrap: true,
                      //       itemCount: item.variant.length,
                      //       itemBuilder: (context,index){
                      //     return getSizeListItem('', '${item.variant[index].quantity} ${item.variant[index].unit}', '${item.variant[index].price}');
                      //   },
                      ...
                      Container(
                        color: kWhiteColor,
                        child: (item.addons!=null && item.addons.length>0)?ListView.separated(
                            shrinkWrap: true,
