...
class AddressTo extends StatefulWidget{
  ...
  AddressTo(this.vendor_name, this.vendor_id, this.distance, this.senderAddress);
...
class AddressToState extends State<AddressTo>{
  ...
  @override
  void initState() {
    city.text = widget.senderAddress.city;
    pincode.text = widget.senderAddress.pincode;
    state.text = widget.senderAddress.state;
    super.initState();
  }
          ...
          children: [
            ...
            Container(
              ...
              child: GestureDetector(
                onTap: () {
                  showProgressDialog('please wait while we setting your cart',pr);
                  hitServiceCount(pr);
                },
                child: Card(
                  elevation: 2,
  ...
  void hitServiceCount(ProgressDialog pr) async{
    print('${widget.vendor_id}');
    pr.show();
    var chargeList = parcel_listcharges;
    var client = http.Client();
    client.post(chargeList, body: {'vendor_id': '${widget.vendor_id}'}).then((value) {
          ...
          if(cityFetcjList.length>0){
            for(int i =0;i<cityFetcjList.length;i++){
              if(cityFetcjList[i].city_name.toString().contains(city.text)){
                double disForCharge = calculateDistance(widget.senderAddress.lat,widget.senderAddress.lng,lat,lng);
                print('${disForCharge}');
                print('${lat}-${lng}');
                setState(() {
                ...
                if(houseno.text!=null && pincode.text!=null && city.text!=null && landmark.text!=null && address.text!=null && state.text!=null && lat!=null && lng!=null && sendercontact.text!=null && sendername.text!=null){
                  ...
                  Navigator.push(context, MaterialPageRoute(builder: (context) => ParcelDetails(widget.vendor_id,widget.vendor_name,widget.distance,widget.senderAddress,parcelAddress,city_id,charges,disForCharge)));
                }else{
            ...
            // int index = cityFetcjList.indexOf(ChargeListBean('','','',city.text,'','','','','',''));
            // if(index<0){
            //   ChargeListBean chargeListBean = cityFetcjList[index];
            //
            // }
          }
        }
