...
class ParcelCheckOut extends StatefulWidget{
  ...
  ParcelCheckOut(this.vendor_id, this.vendor_name, this.distance, this.senderAddress, this.receiverAddress,this.beanDetails,this.distanced,this.charges,this.cart_id);
...
class ParcelCheckoutState extends State<ParcelCheckOut>{
              ...
              child: Column(
                children: [
                  Row(
                    ...
                    children: [
                      Text('Distance'),
                      Text('${(double.parse('${double.parse('${widget.distanced}').toStringAsFixed(2)}')>1)?double.parse('${double.parse('${widget.distanced}').toStringAsFixed(2)}'):1} KM'),
                    ],),
              ...
              child: GestureDetector(
                ...
                child: Card(
                  ...
                  child: Container(
                    ...
                    child: Text('Proceed to payment',style: TextStyle(fontSize: 18,color: kWhiteColor),),
                  ),
                ),
              ),
            ),
  ...
  void getVendorPayment(dynamic vendorId,ProgressDialog pr,BuildContext context) async {
    pr.show();
    SharedPreferences preferences = await SharedPreferences.getInstance();
    setState(() {
      currency = preferences.getString('curency');
      ...
      if (value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          List<PaymentViaParcel> tagObjs = tagObjsJson
              ...
              .toList();
          double amout = (double.parse('${double.parse('${widget.distanced}').toStringAsFixed(2)}')>1)?(double.parse('${double.parse('${widget.distanced}').toStringAsFixed(2)}')*double.parse('${widget.charges}')):double.parse('${widget.charges}');
          print('${amout} - ${widget.vendor_id} - ${widget.cart_id} - ${widget.charges} - ${double.parse('${widget.distanced}').toStringAsFixed(2)} - ${tagObjs.toString()}');
           Navigator.push(context,MaterialPageRoute(builder: (context) {
             return PaymentParcelPage(widget.vendor_id,widget.cart_id,amout,tagObjs,widget.charges,(double.parse('${double.parse('${widget.distanced}').toStringAsFixed(2)}')>1)?double.parse('${widget.distanced}').toStringAsFixed(2):1);
           }));
        }
      }
    }).catchError((e) {
