...
class PaymentParcelPage extends StatefulWidget {
  ...
  PaymentParcelPage(this.vendor_ids,this.cart_id,this.totalAmount,this.tagObjs,this.charges,this.distance);
...
//                    Navigator.popAndPushNamed(context, PageRoutes.orderPlaced),
  ...
  void placedOrder(paymentStatus, paymentMethod) {
    ...
    print('${paymentMethod} - ${iswallet?'yes':'no'} - ${paymentStatus} - ${cart_id} - ${(double.parse('${double.parse('${widget.distance}').toStringAsFixed(2)}')*double.parse('${widget.charges}'))}');
    http.post(url, body: {
      'payment_method': '${paymentMethod}',
      'wallet': iswallet?'yes':'no',
      'payment_status': paymentStatus,
      'cart_id': '${cart_id}',
      'total_price': '${(double.parse('${double.parse('${widget.distance}').toStringAsFixed(2)}')*double.parse('${widget.charges}'))}',
    }).then((value) {
      ...
      if (value != null && value.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          hitNavigator(cart_id, '${paymentMethod}', '${paymentStatus}',
              cart_id, '${(double.parse('${double.parse('${widget.distance}').toStringAsFixed(2)}')*double.parse('${widget.charges}'))}');
        } else {
          setState(() {
            showDialogBox = false;
          });
  ...
  void hitNavigator(cart_id, payment_method, payment_status, order_id,
    ...
    http.post(url, body: {
      ...
      Navigator.of(context).push(MaterialPageRoute(builder: (context) {
        return OrderPlaced(payment_method, payment_status, cart_id, rem_price,currency,"4");
      }));
    }).catchError((e) {
      setState(() {
        showDialogBox = false;
      ...
      Navigator.of(context).push(MaterialPageRoute(builder: (context) {
        return OrderPlaced(payment_method, payment_status, order_id, rem_price,currency,"4");
      }));
    });
  }
